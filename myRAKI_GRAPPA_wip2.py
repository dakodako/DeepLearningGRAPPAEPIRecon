...
# inputData = '../MRI_data/MRI_data/raw_data/raw_data_134_3107_PA/patrefscan_rmos_data_134.mat'
# input_variable_name = 'patrefscan_data_reshaped'
# arrays = {}
# f = h5py.File(inputData)
# for k,v in f.items():
#     arrays[k] = np.array(v)
...
#%%
ACSData = '../MRI_data/MRI_data/raw_data/raw_data_134_3107_PA/refscan_data_rmos_134_3107_PA.mat'
# ACSData = '/home/student.unimelb.edu.au/chid/Documents/MRI_data/MRI_data/raw_data/raw_data_134_3107_PA/pc_rmos_134_meas_15.mat'
ACS_variable_name = 'refscan_data'
arrays = {}
f = h5py.File(ACSData)
for k,v in f.items():
    arrays[k] = np.array(v)
#%%
ACS_full = np.zeros(arrays[ACS_variable_name].shape, dtype=complex)
ACS_full.real = arrays[ACS_variable_name]['real']
ACS_full.imag = arrays[ACS_variable_name]['imag']
#%%
ACS_full = np.transpose(ACS_full,[3,1,2,0])
#%%
data_full = np.transpose(data_full,[3,1,2,0])
#%% don't run this when loading pc corrected data
# data_full_reorder = np.zeros(data_full.shape,dtype=complex)
# data_full_reorder[:,:,:,0:60] = data_full[:,:,:,1::2]
# data_full_reorder[:,:,:,60::] = data_full[:,:,:,0::2]
#%%
...
#%%
# data_full_padded = np.zeros((data_full_reorder.shape[0], data_full_reorder.shape[1]*2-1, data_full.shape[2], data_full_reorder.shape[3]),dtype = complex)
# data_full_padded[:,0::R,:,:] = data_full_reorder
#%%
[NCol,ACS_NRow, NCha, NSlc] = ACS_full.shape
#%%
[NCol, data_NRow, NCha, NSlc] = data_full_padded.shape
#%%
R = 2
#%%
input_data = Input(shape=(NCol,ACS_NRow, NCha*2))
...
for slc in range(NSlc):
    ACS = ACS_full[:,:,:,slc]
    scaling = 1/np.max(np.abs(ACS[:]))
    ACS_scaled = np.multiply(ACS,scaling)
    ACS_re = np.zeros([NCol, ACS_NRow, NCha*2])
    ACS_re[:,:,0:NCha] = ACS_scaled.real 
    ACS_re[:,:,NCha::] = ACS_scaled.imag
    ACS_re = np.expand_dims(ACS_re,0)
    ...
    target_y_end = ACS_NRow  - np.int32((np.floor(kernel_y_1/2) + np.floor(kernel_y_2/2) + np.floor(kernel_last_y/2))) * R -1
    ...
    for ch in range(NCha*2):
        ...
        for ind_acc in range(R-1):
            ...
            target_y_end = ACS_NRow  - np.int32((np.floor(kernel_y_1/2) + (np.floor(kernel_y_2/2)) + np.floor(kernel_last_y/2))) * R + ind_acc
            target[0,:,:,ind_acc] = ACS_re[0,target_x_start:target_x_end + 1, target_y_start:target_y_end +1,ch]
        ...
        recon_model_train = recon_model.fit(ACS_re, target, epochs=1000,verbose=1,batch_size=1)
